...
class ToDo extends React.Component {
    ...
    return this.state.items.map((item) => (
      <Li key={item.id}>
        <FlexDiv>
          ...
          ></Span>
          <p>
            {!item.is_completed === true && item.id},
            {!item.is_completed === true && item.title}
          </p>
        </FlexDiv>
  ...
  completedItems = (id, title) => {
    ...
    axios
      .post(`http://127.0.0.1:8000/api/v1/tasks/update/${id}/`, {
        is_completed: true,
      })
      ...
      <Li style={{ color: "#0cc694" }} key={item.id}>
        ...
        <small>
          ...
          <DeleteSvg
            ...
            onClick={() => this.removeFinishedItems(item.id)}
          />
        </small>
      </Li>
    ));
